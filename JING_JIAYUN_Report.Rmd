...
```{r PartI,echo=FALSE,message=FALSE, warning=FALSE}
set.seed(611)
...
#### Response Variable
- **`diabetes`**: A binary variable (`1` = diabetic, `0` = non-diabetic).  
  Only **8.5%** of individuals have diabetes, indicating a **strong class imbalance** that must be considered during modeling.
...
num_table <- data.frame(
  Variable = c("Age", "BMI", "HbA1c_level", "Blood_Glucose_Level"),
...
kable(num_table, format = "latex", booktabs = TRUE, linesep = "",caption = "Association of Numerical Variables with Diabetes") %>%
  kable_styling(position = "center", font_size = 9,latex_options = "hold_position" ) %>%
  column_spec(3, width = "6.5cm")
```


```{r, echo=FALSE, fig.width=6, fig.height=4.5,out.width="90%",fig.pos='H'}
...
kable(cat_table, format = "latex", booktabs = TRUE,caption = "Association of Categorical Variables with Diabetes") %>%
  kable_styling(position = "center", font_size = 9,latex_options = "hold_position") %>%
  column_spec(3, width = "6.5cm")
```
...
X = data_knn[, c("age", "hypertension", "heart_disease", "bmi", 
...
plot(k_values, mean_type1_error_knn, type = 'b',
     ...
     ylab = "Average Type I Error",
     main = "KNN Tuning: Type I Error vs. k")

abline(v = best_k, col = "red", lty = 2)
```
### 2.2 Decision Tree (DT)
We first tuned the Decision Tree by minimizing the **cross-validated Type I Error**, but this caused a trivial classifier that always predicted the majority class due to severe imbalance (only 8.5 % positive cases).
To address this, we switched to optimizing **the Geometric Mean (G-Mean)**, which balances sensitivity and specificity. This approach yielded the optimal hyperparameters: **cp = 0.001, minsplit = 5, and maxdepth = 30**.

```{r echo=FALSE, fig.width=10, fig.height=2.5, fig.align='center', out.width="100%"}
...
knitr::kable(type1_error_lr, digits = 4, caption = "Comparison of LR Models (Type I Error)") %>%
...
knitr::kable(comparison_df, digits = 4, caption = "Best LR Model: Default vs Optimal Thresholds") %>%
  kable_styling(latex_options = "scale_down")
```
...
knitr::kable(summary_table, booktabs = TRUE,
             col.names = c("Model", "AUC", "TPR", "Precision"),caption = "Final Performance of KNN, DT, and LR")
```
...
pros_cons_data <- tribble(
  ...
  "\\textbf{High Precision (98.22\\%):} Positive predictions are extremely reliable, useful for confirmatory diagnosis. \n\n
   \\textbf{Model Simplicity:} Conceptually simple and non-parametric.",
  "\\textbf{Poor Sensitivity (58.40\\%):} Fails to identify over half of diabetic cases, making it unsuitable for initial screening. \n\n
  ...
  "Decision Tree (DT)",
  "\\textbf{Interpretability:} Rules are easy for medical professionals to understand and validate. \n\n
   \\textbf{Handles Mixed Data Types:} Natively uses both numerical and categorical predictors. \n\n
   \\textbf{Balanced Performance:} Achieved a good balance between sensitivity (66.91\\%) and precision (100.00\\%).",
  ...
  "\\textbf{Highest Sensitivity (88.94\\%):} Best at identifying true diabetic cases, which is the primary goal of screening. \n\n
  ...
  "\\textbf{Low Precision (39.19\\%):} High sensitivity comes at the cost of many false positives. \n\n
